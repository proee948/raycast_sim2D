...
int menu(SDL_Renderer *renderer, double *ray_len)
{
    ...
    SDL_Rect submenu5 = {.h = (MENU_HEIGHT / 12), .w = (MENU_WIDTH / 12), .x = submenu1.x ,.y = (submenu1.y + submenu1.h) + Y_MENU_SECTION_RAZMAK};
    ...
    t2 = SDL_CreateTextureFromSurface(renderer,surface);
    SDL_FreeSurface(surface);

    surface = SDL_LoadBMP("arrow_left.bmp");
    t3 = SDL_CreateTextureFromSurface(renderer,surface);
    SDL_FreeSurface(surface);

    surface =  TTF_RenderUTF8_Solid(font,"RAY LENGHT",txt_col);
